...
class TouchPointCon(ExplicitComponent):

    def setup(self):
        
        self.add_input('x')
        self.add_input('y')
        self.add_output('touch_dist')
    ...
    def compute(self, inputs, outputs):
        ...
        TestPoint_x = trajectory_x[99]
        TestPoint_y = trajectory_y[99]

        outputs['touch_dist'] = np.linalg.norm([x - TestPoint_x, y - TestPoint_y])
...
if __name__ == '__main__':
    ...
    comp = TouchPointCon()
    group.add_subsystem('touch_con', comp, promotes=['*'])
