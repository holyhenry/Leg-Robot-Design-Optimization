...
class SwingPointCon(ExplicitComponent):

    def setup(self):
          
        self.add_input('x')
        self.add_input('y')
        self.add_output('swing_dist')
    ...
    def compute(self, inputs, outputs):
        ...
        TestPoint_x = trajectory_x[79]
        TestPoint_y = trajectory_y[79]

        outputs['swing_dist'] = np.linalg.norm([x - TestPoint_x, y - TestPoint_y])
...
if __name__ == '__main__':
    ...
    comp = SwingPointCon()
    group.add_subsystem('swing_con', comp, promotes=['*'])
